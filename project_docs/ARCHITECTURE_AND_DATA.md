...
## Implemented PH-05 optional in-app reminder settings

`BR-20260923-03` adds `plan_schedule_reminder_settings`, one optional immutable lead time (15 through 10080 minutes) per exact `plan_schedule_versions` row. A missing settings row means reminders are off; earlier schedule versions remain unchanged. The authenticated transactional `save_plan_schedule_with_reminder` RPC delegates to the existing versioned `save_plan_schedule` authority and stores the new setting only after the new schedule snapshot is created. Existing save/RLS/stale-write/constraint-validation semantics are preserved; the prior RPC remains available for compatibility and produces no reminder setting.

The existing `get_my_plan_schedule_occurrences` function now appends `reminder_minutes_before` and `reminder_at` to its original 18 output columns, preserving earlier result-column order. The reminder timestamp is derived from the effective occurrence start, so skip suppresses the occurrence and reminder, reschedule moves both, restore returns both and pause suppresses current projections. `reminderDue` is computed in the server scheduling read layer rather than inside React render. No durable delivery attempt, acknowledgment or notification queue is created; opening Today after a due time does not constitute background delivery.

`plan_schedule_reminder_settings` uses owner-only RLS and select grants, revokes direct authenticated writes, has immutable-update protection and cascades through the owning schedule-version lifecycle. Historical reminder lead times are included in readable account export v9. No browser notification permission, push subscription, email/SMS provider, background worker, recurring hosted job or external cost is introduced. If actual delivery while the application is closed becomes required, specify delivery semantics, timezone and provider/privacy/deployment requirements before adding that mechanism (`RISK-028`). Progression remains later PH-05 work.

## Implemented locally: PH-05 conservative review-only progression prerequisite

`BR-20260924-01` adds `plan_progression_reviews` (immutable owner-private exact plan/schedule-version review snapshots) and `plan_progression_review_events` (append-only `proposed`/`dismissed` events). Both use owner-select RLS, revoked direct authenticated mutation, immutable-update triggers and Auth-user deletion through cascading plan/review relationships. Privileged RPCs `create_conservative_plan_progression_review` and `dismiss_conservative_plan_progression_review` derive the authenticated user, enforce plan ownership and expected source/event versions, and reject stale requests. Repeating identical still-pending feedback for the same exact schedule returns the existing review without duplicating history.

Only explicit user feedback is accepted: `excessive_difficulty`, `discomfort` or `user_requested_reduction`. With multiple weekly scheduled days, difficulty or requested reduction requires choosing a scheduled weekday and previews reduction by exactly one day. Discomfort, or a one-day schedule, previews a pause (zero scheduled days). The preview records old/new weekly frequency, a reason category, the selected weekday when relevant, and the exact immutable source plan/schedule IDs. No diagnosis, exercise dosage escalation, inferred completion, automatic schedule change or AI is involved. Dismissal appends an event; it does not erase history or reverse an applied change.

Plan detail offers a clearly marked non-applied proposal and explicit dismissal; it prevents new review while paused or when the schedule points to an older plan version. Owner-readable JSON export advances to v10 with each plan's full progression proposal/event history. `REQ-021` (plan-level only), `REQ-022` (preview/explanation/dismissal only) and `REQ-023` (explicit difficulty/discomfort/reduction only) are partial. Exercise-/routine-level progression, proposal application and true reversal, missed-session handling and remaining PH-05 acceptance are not implemented.
