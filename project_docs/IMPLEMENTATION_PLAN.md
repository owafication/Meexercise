...
**Validation:** `VAL-028`.
**Current evidence:** `BR-20260923-01` locally verifies the first scheduling slice: stable owner-private schedule identity per plan; immutable append-only schedule versions pinned to the current exact plan version; one local-time routine window per selected weekday; recognized named timezone storage; daylight-saving-safe occurrence projection; expected plan/schedule version conflict checks; versioned pause; owner-only schedule reads; Today and plan-detail upcoming projections; export v7 schedule history; account-deletion cascade; focused 33-test pgTAP plus complete pgTAP and authenticated browser regression. `REQ-020` / `AC-012` remain partial because skip/reschedule and reminders are not yet implemented. Progression `REQ-021`-`REQ-023` is untouched. Remote exact-head CI/merge is Unproven, so PH-05 remains Active / In progress.
