...
**Validation:** `VAL-028`.
**Current evidence:** `BR-20260923-01` weekly recurrence/timezone/pause foundation is merged and remotely verified through exact-head CI #51 and PR #28. `BR-20260923-02` locally runtime-verifies immutable append-only per-occurrence skip/reschedule/restore history pinned to an exact schedule rule and original local date, stale exception-write rejection, exception-aware occurrence projection, explicit preservation of the exact routine version, no silent exception inheritance into a newly saved recurring schedule version, owner isolation, account-deletion cascade and export v8. Focused 39-test exception pgTAP, complete pgTAP and complete authenticated browser regression pass. `REQ-020` / `AC-012` remain partial only because reminder capability is not yet implemented. Progression `REQ-021`-`REQ-023` remains untouched. Exact-head remote CI/merge for this second slice is Unproven, so PH-05 remains Active / In progress.
