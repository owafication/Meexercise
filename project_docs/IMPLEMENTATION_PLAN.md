...
**Validation:** `VAL-028`.
**Current evidence:** `BR-20260923-01` weekly recurrence/timezone/pause is remotely verified through CI #51 / PR #28. `BR-20260923-02` skip/reschedule/restore is remotely verified through exact-head CI #53 / PR #29 at merge `8416a43472fabeb141006a172f9e2e8a42df51e7`. The third locally runtime-verified scheduling slice (`BR-20260923-03`) adds an optional immutable in-app reminder lead time (15 minutes to 1 week) attached to each exact schedule version, effective occurrence reminder projection, transparent Today due indication, owner RLS/history/cascade and account export v9. The 27-test focused pgTAP, full database regression, lint, migration/seed replay and 17 authenticated browser tests passed. `REQ-020` reminder support is limited to in-app visibility on app load; background notifications or delivery guarantees have not been specified, implemented or verified. No queue, scheduler or push provider is justified yet (`RISK-028`). Progression `REQ-021`-`REQ-023` is untouched; PH-05 stays Active / In progress and this third slice still needs exact-head remote CI/merge.
