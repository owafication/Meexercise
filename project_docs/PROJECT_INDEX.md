...
**Mode of latest record:** PH-04 exact-version exercise planning-metadata prerequisite implementation and isolated local verification
**Repository state:** PH-00/PH-01/PH-02/PH-03 are verified on merged `main`. PH-04 manual foundation, ordered editing, structured movement constraints, manual constraint consumption, deterministic guided proposal/review and structured planning-profile capture are merged; PR #22 exact head `b6f72a292d954fc1b33392127b10d9e71142aa1c` merged as `4dbb02e00368cd435012c1fe18e18922d5b90f06` after exact-head CI run #39 passed both `Verify` and `Database and auth integration`. The current `agent/ph04-exercise-planning-metadata` slice locally verifies immutable exact-version planning metadata for accepted goals, methods, equipment, facilities and estimated minutes, with explicit classification completeness required before finalisation. The server exercise library exposes these structured fields without changing guided selection behaviour. Deterministic generator consumption of the complete private planning profile and unlimited templates remain unimplemented/unproven (`BR-20260917-01`). Runtime AI remains unused.
...
- `PH-04` — **In progress / six slices merged; exact-version exercise planning-metadata prerequisite locally verified**: manual create/edit, append-only routine versioning, structured movement constraints, manual constraint consumption, deterministic guided proposal/review and the structured `REQ-003` planning profile are merged. The current local slice adds explicit exact-version planning goal tags, method tags, equipment tokens, facility tokens, deterministic estimated minutes and a classification-complete marker to `exercise_versions`. Draft content cannot finalise until this metadata is complete, and planning metadata is immutable after finalisation. Synthetic fixtures are explicitly classified to prove mechanics only; no title/prose inference or production catalogue claim is made. The server library exposes the metadata for the next deterministic consumer. `REQ-014` remains partial until guided generation requires a complete planning profile and consumes these fields while preserving current readiness/constraint/approval gates. Unlimited templates (`REQ-018`/`AC-011`), this slice exact-head remote CI/merge and PH-04 closure remain Unproven. Runtime AI remains absent.
