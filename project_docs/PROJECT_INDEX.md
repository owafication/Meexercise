...
**Mode of latest record:** PH-04 structured movement-constraint prerequisite implementation and isolated local verification
**Repository state:** PH-00/PH-01/PH-02/PH-03 are verified on merged `main`. PH-04 first and second slices are merged: PR #18 exact head `84a30d52234ef32d8f39888ce3d413b699fc4271` merged as `2ccd6fa867224d12737be878fa79c9cfacf658da` after exact-head CI run #31 passed both `Verify` and `Database and auth integration`. The PH-04 structured-constraint prerequisite is locally verified on `agent/ph04-structured-constraints`: readiness template v2 records optional structured movement-avoidance choices; exact exercise versions own immutable planning constraint tags; private deterministic compatibility/substitution primitives use approved exact versions and existing version-owned relations; free text is not interpreted. Restricted routine create/edit intentionally remains fail-closed until the next PH-04 consumer slice (`BR-20260824-01`).
...
- `PH-04` — **In progress / first + second slices merged; structured-constraint prerequisite locally verified**: ordered manual create/edit and append-only routine versioning are merged. The current local slice extends `REQ-005` with optional structured movement-avoidance choices on readiness template v2 and partially advances `REQ-010`/`REQ-014`, `AC-005`/`AC-006`, and `VAL-027` through exact-version immutable constraint metadata, deterministic compatibility checks and compatible lookup through existing substitution relations. Legacy/readiness-v1 or unclear restrictions remain fail-closed; no free-text restriction inference exists; restricted routine save/edit remains blocked until the next consumer slice. `REQ-014`–`REQ-016`, full deterministic restricted-planning consumption, templates and this slice exact-head remote CI/merge remain Unproven.
