...
**Mode of latest record:** PH-04 profile-aware deterministic guided-generation consumption and isolated local verification
**Repository state:** PH-00/PH-01/PH-02/PH-03 are verified on merged `main`. Seven PH-04 slices are merged through PR #23: manual routine foundation, ordered editing, structured movement constraints, manual constraint consumption, deterministic guided proposal/review, structured planning-profile capture and exact-version exercise planning metadata. PR #23 exact head `100614e3d6c8c92df9b0425fc08c5b2e050e4076` merged as `2e9652871f6fc53a84dec85af275f69c6860e790` after exact-head CI run #41 passed both `Verify` and `Database and auth integration`. The current `agent/ph04-profile-generation-consumption` slice locally verifies deterministic guided generation that requires a complete private planning profile and consumes primary/secondary goal priority, preferred methods, equipment, facilities, available routine time and preferred weekly frequency context against immutable exact-version metadata while preserving readiness constraints and explicit item review. Guided save rechecks the current profile before persistence. Unlimited templates and PH-04 closure remain unimplemented/unproven (`BR-20260918-01`). Runtime AI remains unused.
...
- `PH-04` — **In progress / seven slices merged; profile-aware guided generation locally verified**: manual create/edit, append-only routine versioning, structured movement constraints, manual constraint consumption, deterministic guided proposal/review, structured `REQ-003` planning-profile capture and exact-version exercise planning metadata are merged. The current local consumer requires a complete planning profile before guided generation; primary goal is a hard eligibility filter, secondary goal is a deterministic tie-break, preferred methods/facilities require intersection, required equipment must be available, and summed exact-version estimated minutes must fit the available routine-time budget. Preferred weekly frequency is consumed and explained as planning context without claiming scheduling. Existing readiness constraints and approved exact-version visibility remain mandatory, replacement choices stay within the proposal structure/time/profile rules, and the guided-save boundary rechecks the current profile so a stale proposal fails closed. Manual routine creation remains user-directed and is not forced to match preferences. `REQ-014`, `REQ-015`, `REQ-016`, `AC-006` and `AC-007` are locally advanced/verified for this guided-routine scope; exact-head remote CI/merge for this slice, unlimited templates (`REQ-018`/`AC-011`) and PH-04 closure remain Unproven. Runtime AI remains absent.
- `PH-05` through `PH-10` - **Proposed**.
