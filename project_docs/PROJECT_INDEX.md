...
**Mode of latest record:** PH-04 manual routine foundation first-slice implementation and isolated local verification
**Repository state:** PH-00/PH-01/PH-02/PH-03 are verified on merged `main`; PH-03 canonical closure PR #16 merged as `b5755f076af709905f9f5aeb7fbdf0a3d415ce5c`. PH-04 first slice is implemented and locally verified on `agent/ph04-manual-routine-foundation`: private manual routine snapshots retain exact exercise-version IDs, readiness/content creation gates fail closed, owner-only list/detail surfaces are present, export v3 includes routine history, and account deletion cascades through routine records. PH-04 remains In progress because manual editing/new routine versions, deterministic restriction matching/substitutions, guided generation/explanations/user review and templates are not yet implemented (`BR-20260821-01`).
...
- `PH-04` — **In progress / first slice locally verified**: manual routine creation now persists private stable routine identities with immutable version-one snapshots, sections/items and exact approved exercise-version references; `/create`, `/plans` and `/routines/[routineId]` expose the first owner-only planning flow; readiness/content gates fail closed; export v3 and account-deletion cascade cover routine data. `REQ-012` is locally verified for saved routine snapshots; `REQ-013` is partial because editing/new routine versions are not yet implemented. `REQ-014`–`REQ-016`, broader `REQ-019`, deterministic restriction matching/substitution, templates and PH-04 exact-head remote CI/merge remain Unproven.
