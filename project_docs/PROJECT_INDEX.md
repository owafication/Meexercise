...
**Mode of latest record:** PH-05 progression review-only slice, local verification; remote publication outstanding
...
**Current PH-05 evidence:** `BR-20260923-01` and `BR-20260923-02` are published (PR #28/#29). `BR-20260923-03` in-app reminders were also published: reminder head `036f1ec5afa6aa9159cc0010b92a260937242a13` completed GitHub CI run #55 (`35833474112`) successfully and PR #30 merged to `main` at `b50d92ca13a4bca2b19a9a0e2209a7b636ddf28c` on 2026-09-23. `BR-20260924-01` records local-only progression review proposals on `agent/ph05-progression-review` at this baseline: explicit difficulty/discomfort/user-requested reduction; exact source plan/schedule snapshot, conservative pause or chosen-day frequency-reduction preview, immutable proposal/dismissal event history, owner controls/RLS, export v10 and account deletion. Locally, focused progression pgTAP passed 25/25; complete authenticated Playwright passed 18/18; lint (two existing unused-parameter warnings), typecheck, 21 unit/component tests and production build passed; complete preserved-volume pgTAP passed 366/366 after scoping three earlier lifecycle assertions to their own fixtures; database lint found no errors. Clean local replay of the new progression migration and progression exact-head remote CI/merge are Unproven. Review recording/dismissal never applies or reverses a schedule change. Missed-session feedback and background reminder delivery remain outside this slice; `REQ-021`–`REQ-023`, `AC-009`/`AC-010`, `VAL-028` and PH-05 are partial.
...
- `PH-05` - **Active / In progress**: merged `main` now includes weekly recurrence/timezone/pause, per-occurrence exceptions and optional versioned in-app reminder visibility (PR #28/#29/#30, latest merge `b50d92ca13a4bca2b19a9a0e2209a7b636ddf28c`). `BR-20260924-01` records a locally verified, uncommitted first progression slice: user-initiated review-only conservative pause or selected-day reduction proposals, immutable dismissal events, owner privacy, export v10 and primary-store cascade coverage. No proposed change is automatically applied to the current schedule; apply/reverse controls, missed-session input, closed-app reminders, remaining progression work and exact-head CI/merge for this slice are outstanding.
