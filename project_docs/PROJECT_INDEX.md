...
**Mode of latest record:** PH-05 in-app schedule reminder local runtime verification
...
**Current PH-05 evidence:** `BR-20260923-01` weekly recurrence/timezone/pause was exact-head green in CI #51 and merged through PR #28. `BR-20260923-02` per-occurrence skip/reschedule/restore head `e34b807ad66b374da4540fca648ff84ab087a6e9` passed both required jobs in CI #53 (`35822901819`) and PR #29 merged as `8416a43472fabeb141006a172f9e2e8a42df51e7`. `BR-20260923-03` records the uncommitted third slice on `agent/ph05-schedule-reminders` at that exact baseline: immutable per-schedule-version optional in-app lead times, effective-occurrence reminder projection, due indicator on Today, owner isolation and export v9. Clean migration/seed replay, database lint, 27 focused pgTAP tests, complete pgTAP and all 17 authenticated browser tests passed locally. Remote CI/merge for the third slice is Unproven. This is app-view reminder state, not background push/email/SMS delivery; broader delivery requirements remain unresolved. Progression `REQ-021`-`REQ-023` remains later PH-05 work.
...
- `PH-05` - **Active / In progress**: merged `main` contains exact-head verified weekly recurrence/timezone/pause (PR #28) and per-occurrence skip/reschedule/restore (PR #29). The third slice (`BR-20260923-03`) is locally runtime-verified only: optional versioned in-app reminder lead times, reminder timestamps derived from effective occurrences, Today due indication and export v9. This further advances `REQ-017` / `REQ-020` and `VAL-028`; `AC-012` schedule mechanics are covered by the merged earlier slices. Background reminder delivery is not implemented or claimed, and full reminder expectations beyond in-app visibility require a separate delivery decision. Progression `REQ-021`-`REQ-023` remains unimplemented; exact-head remote CI/merge for the third slice is still Unproven.
...
- BR-20260923-03: PH-05 in-app reminders implementation/local runtime-verification record; current third slice remains uncommitted pending remote CI and PR.
