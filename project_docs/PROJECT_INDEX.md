...
**Mode of latest record:** PH-04 deterministic guided proposal/review implementation and local verification
**Repository state:** PH-00/PH-01/PH-02/PH-03 are verified on merged `main`. PH-04 manual foundation, routine editing, structured-constraint prerequisite and manual constraint-consumer are merged; PR #20 exact head `f67fd24b71f110cc995330085048a8bb61ca22c0` merged as `54ce10def372cdb63df3d25bbb5524d940a70511` after exact-head CI run #35 passed both `Verify` and `Database and auth integration`. The current guided-proposal slice is locally verified on `agent/ph04-guided-proposal`: deterministic bounded generation uses only approved compatible exact exercise versions, explains purpose/balance/constraints/substitutions, exposes every proposed item for explicit replacement before save, and persists only the reviewed selection through the existing authoritative routine mutation. Runtime AI and free-text interpretation remain absent. Full profile goals/preferences/equipment/facilities/available-time/frequency consumption and unlimited templates remain unimplemented (`BR-20260910-01`).
...
- `PH-04` — **In progress / four slices merged; guided proposal/review locally verified**: manual create/edit, append-only routine versioning, structured constraint primitives and manual constraint consumption are merged. The current slice adds bounded deterministic guided generation on `/create`: only approved compatible exact exercise versions enter a proposal; balanced/upper/lower focus and 1–6 item count are deterministic inputs; explanations cover purpose, balance, active structured constraints and reviewed substitutions; every proposed item has an explicit review/replacement selector before save; final persistence reuses the existing authoritative exact-version routine mutation. `REQ-014` advances partially; the implemented guided surface locally verifies current `REQ-015`/`REQ-016` and `AC-007` scope. Runtime AI, automatic substitution and free-text constraint inference remain absent. Full `REQ-014` profile-goal/equipment/time/frequency consumption, unlimited templates (`REQ-018`/`AC-011`) and this slice exact-head remote CI/merge remain Unproven.
