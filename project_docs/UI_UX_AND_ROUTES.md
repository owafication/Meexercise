...
The current library is read-only. It provides semantic search plus equipment/target-area filters, exposes only currently visible `general`/`reviewed` exercise versions, and renders structured setup/steps/cues/dosage/common-errors/safety/plain-language content plus visible related variation links. Draft/withdrawn/restricted and future professional-authoring surfaces are not exposed. `/create` links to the library but routine construction remains PH-04.
