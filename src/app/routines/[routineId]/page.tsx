...
export default async function RoutineDetailPage({ params }: Props) {
  ...
  return (
    <>
      <PageIntro
        eyebrow={`Routine · version ${routine.versionNumber}`}
        title={routine.title}
      >
        <p>
          This saved version references the exact exercise versions shown below.
          Editing creates a new routine version rather than rewriting this
          snapshot.
        </p>
      </PageIntro>

      <div className="action-row">
        <Link className="button" href={`/routines/${routine.id}/edit`}>
          Edit routine
        </Link>
        <Link className="button button-secondary" href="/plans">
          Back to Plans
      ...
      {routine.sections.map((section) => (
        <section
          className="card"
          aria-labelledby={`routine-section-${section.id}`}
          key={section.id}
        >
            ...
            {section.items.map((item) => (
              <li key={item.id}>
                <article>
                  ...
                  <p>
                    Exact exercise version {item.exerciseVersion.versionNumber} ·{" "}
                    {statusText(item.exerciseVersion.status)}
                  </p>
                  <p>{item.exerciseVersion.summary}</p>
                  <p>
                    <strong>Dosage guidance:</strong>{" "}
                    {item.exerciseVersion.dosageGuidance}
                  </p>
                  <p>
                    <strong>Side rule:</strong>{" "}
                    {statusText(item.exerciseVersion.sideRule)}
                  </p>
                  <p>
                    <strong>Plain-language description:</strong>{" "}
                    {item.exerciseVersion.accessibleText}
                  </p>
                </article>
              </li>
            ))}
